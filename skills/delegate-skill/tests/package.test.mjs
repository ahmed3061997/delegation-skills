/**
 * delegate-skill · tests/package.test.mjs
 *
...
test("SKILL.md carries the metadata a skill host needs", () => {
  ...
  equal(fields.name, "delegate-skill");
...
test("catalog.mjs exits 0 with nothing installed and 2 on a bad flag", () => {
  ...
  includes(run("catalog.mjs", ["--path"]).stdout, "delegate-skill/catalog.json");
});
...
test("select.mjs emits the JSON a native question tool needs", () => {
  ...
  equal(agents.schema, "delegate-skill.agent-choices.v1");
  ...
  equal(models.schema, "delegate-skill.model-choices.v1");
  ...
  equal(efforts.schema, "delegate-skill.effort-choices.v1");
