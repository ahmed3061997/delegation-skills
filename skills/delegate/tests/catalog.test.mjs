/**
 * delegate · tests/catalog.test.mjs
 *
...
test("the cache path honours XDG_CACHE_HOME", () => {
  equal(catalogPath({ XDG_CACHE_HOME: "/tmp/xdg" }), "/tmp/xdg/delegate/catalog.json");
  includes(catalogPath({}), join(".cache", "delegate", "catalog.json"));
});
